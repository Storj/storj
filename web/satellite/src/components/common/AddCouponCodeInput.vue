...
export default class AddCouponCodeInput extends Vue {
    ...
    @Prop({ default: () => null })
    public readonly setParentCouponCode: (couponCode: string) => void;
    @Prop({default: false})
    ...
    /**
    * Toggles showing of coupon code switch confirmation
    */
    ...
    /**
     * Check if user has coupon codes before applying
     */
    public async couponCheck(): Promise<void> {

        try {
            let userCouponCodes = await this.payments.listCouponCodes();

            if(userCouponCodes.length > 0) {
                this.toggleConfirmMessage()
            } else {
